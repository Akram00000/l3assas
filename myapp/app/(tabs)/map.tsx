...
import { useGlobalAlert, useSettings, useStatusPoll, useVisionPoll } from '@/src/hooks/appHooks';
...
export default function MapScreen() {
  ...
  const globalAlert = demo.statusOverride?.global_alert ?? status.data?.global_alert ?? computedGlobalAlert;
  ...
  const alertSource = useMemo<AlertSource>(() => {
    const visionAlert = demo.statusOverride?.vision_alert ?? status.data?.vision_alert ?? visionAlertRaw;
    const sensorAlert = demo.statusOverride?.sensor_alert ?? status.data?.sensor_alert ?? sensorAlertRaw;

    if (visionAlert !== 'CLEAR') return 'vision';
    if (sensorAlert !== 'SAFE') return 'sensor';
    return null;
  }, [
    demo.statusOverride?.sensor_alert,
    demo.statusOverride?.vision_alert,
    sensorAlertRaw,
    status.data?.sensor_alert,
    status.data?.vision_alert,
    visionAlertRaw,
  ]);
  ...
  useEffect(() => {
    ...
    setActiveCameraLabel(null);
  }, [alertSource, demo.statusOverride?.alert_updated_at, globalAlert, status.data?.alert_updated_at]);
        ...
        <ScrollView
          ...
          <Card>
            ...
            {demo.enabled && !demo.fireTriggered && (
              <View style={stylesScoped.demoCountdownBadge}>
                <Text style={stylesScoped.demoCountdownText}>{t.demoModeCountdown}: {demo.countdownSeconds}</Text>
              </View>
            )}
            ...
            <View
              ...
              <View
                ...
                <Text style={[stylesScoped.hotZoneState, { color: colors.text }]}>
                  {globalAlert === 'FIRE' ? t.fireDetected : globalAlert === 'WARNING' ? (isIntrusionWarning ? t.intrusionDetected : t.potentialFireAlert) : t.safe}
                </Text>
              </View>
